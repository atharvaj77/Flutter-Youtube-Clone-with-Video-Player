      ...
      home: Scaffold(
        appBar:AppBar(
          title: Row(children: <Widget>[
            Row(children: <Widget>[
            Image.asset('assets/icons/youtube.png',width: 40, height: 30,),
            Text('Youtube',style: TextStyle(color:Colors.black,fontFamily:'BebasNeue'),
            ),
          ],
          ...
          Row(children: <Widget>[
            Row(children:<Widget>[
              GestureDetector(child: Icon(Icons.videocam, color: Colors.black,),),
              SizedBox(width: 10),
              GestureDetector(
                onTap:  (){Navigator.push(context, new MaterialPageRoute(builder: (context)=> new Search()));},
                child: Icon(Icons.search, color: Colors.black)),
              SizedBox(width: 10),
              GestureDetector(
                onTap: (){Navigator.push(context, new MaterialPageRoute(builder: (context)=> new Account()));},
                child: Image.asset('assets/icons/user.png', width: 40, height: 30,))
            ],
...
class Data{
  ...
  String videoViews;
  String uploadedago;
  Data({this.channelLogo,this.channelName,this.imgLoc,this.title,this.uploadedago,this.videoViews});
}
...
class TileElement extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (){Navigator.push(context, new MaterialPageRoute(builder: (context) => new VideoPlayerScreen()));},
      child: Container(
          ...
          children:<Widget>[
            ...
            Row(children:<Widget>[
              ...
              Column(
                ...
                children:<Widget>[
                  Text(data.title, style: TextStyle(fontWeight: FontWeight.w500),),
                  SizedBox(height: 20),
                  Row(children:<Widget>[
                      Text(data.channelName, style: TextStyle(color: Colors.grey),),
                      Text('.'),
                      Text(data.videoViews, style: TextStyle(color: Colors.grey),),
                      Text('.'),
                      Text(data.uploadedago, style: TextStyle(color: Colors.grey),)
                  ],
